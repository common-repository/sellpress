...
final class SellPress_Route{
    ...
    public static function match($methods, $route, $callable){
        ...
        foreach($methods as $method){
            if(!in_array(strtoupper($method), array('ANY', 'GET', 'POST', 'HEAD', 'PUT', 'DELETE'))){
    ...
    private function hook(){
        if(!$this->hooked){
            add_filter('template_include', array('SellPress_Route', 'onInit'), 1, 0);
            $this->hooked = TRUE;
        }
    }
